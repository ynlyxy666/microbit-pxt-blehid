...
void HIDService::pm_events(const pm_evt_t* p_event) {
  ...
  if(p_event->evt_id == PM_EVT_PEER_DATA_UPDATE_SUCCEEDED) {
    ...
    // Only check the media report characteristic
    int i = mbbs_cIdxReport1;
    int idx = 0;
    ...
    // Update the reporter
    if(reporters[idx]) {
      reporters[idx]->setEnabled(value ? true : false);
    }
  ...
  timeBetweenNotifies(defaultTimeBetweenNotifies)
{
  ...
  // Only create one report characteristic for media
  int idx = 0;
  int i = mbbs_cIdxReport1;
  ...
  addReportDescriptor(charHandles(i)->value, 1, 1 /* Input report */);
...
void HIDService::onDisconnect( const microbit_ble_evt_t *p_ble_evt)
{
    ...
    // Only disable the media reporter
    if(numReporters > 0 && reporters[0]) {
      reporters[0]->setEnabled(false);
    }
}
...
void HIDService::onDataWritten( const microbit_ble_evt_write_t *params)
{
    ...
    if(index == mbbs_cIdxReport1 && type == microbit_charattrCCCD) {
      DEBUG("HID Report CCCD Changed\n");
      bool status = params->len>0 && params->data[0] ? true : false;
      int reporterIdx = 0; // Only one reporter (media)
...
void HIDService::debugAttribute(int handle) {
      ...
      if(index == mbbs_cIdxReport1) 
      {
          int report = 0; // Only one reporter (media)
          index=3;
          if(reporters[report]) {
